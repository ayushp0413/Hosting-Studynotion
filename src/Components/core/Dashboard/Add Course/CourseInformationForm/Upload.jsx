...
export default function Upload({name, label, register, setValue, errors, video = false, viewData = null, editData = null,}) {
  ...
  const inputRef = useRef(null); // dosent really required

  const onDrop = (acceptedFiles) => {
    const file = acceptedFiles[0]
    if (file) {
      setSelectedFile(file)
      previewFile(file); // function call
    }
  }
  ...
  return (
    <div className="flex flex-col space-y-2">
      ...
      <label className="text-sm text-richblack-200" htmlFor={name}>
          ...
          <div className="flex w-full  flex-col p-6">
            {!video ? (
              <img
                src={previewSource}
                alt="Preview"
                className="w-full rounded-md object-cover bg-pink-300"
